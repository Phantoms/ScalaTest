...
class ConvertFromCurrencyToEuroUseCaseTest extends AnyFunSuite {
  test("All") {
    ...
    val useCase = new ConvertFromCurrencyToEuroUseCase(mockDataResource)

    // Happy path
    useCase("USD", "2000") should equal(Right(FromCurrencyToEuroResponse("USD", "EUR", 2000.0, 1000.0)))
